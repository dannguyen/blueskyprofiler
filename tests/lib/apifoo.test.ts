...
describe('apifoo Functions', () => {
  describe('getProfile', () => {
    ...
    it('should fetch profile data from the API', async () => {
      ...
      expect(fetch).toHaveBeenCalledWith(`${API_HOST}/xrpc/app.bsky.actor.getProfile?actor=${handle}`);
    ...
    it('should throw an error when the API request fails', async () => {
      ...
      await expect(getProfile('non-existent-user')).rejects.toThrow('API request failed with status 404');
    });
  });
});
